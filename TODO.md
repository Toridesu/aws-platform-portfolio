...
- [x] READMEに最終的な実行手順と検証結果を整理する
- [x] ポートフォリオとして見せる用の説明文を整理する
- [x] READMEの重複を削減し、詳細ドキュメントへ分離する
- [x] READMEの構成図を画像生成した図へ置き換える
- [x] 公開用ドキュメントを設計レビュー向けに再構成する
- [x] 自分用ドキュメントをGit管理外の `private_docs/` へ整理する
...
- 実行したコマンドや学習用メモは `private_docs/phase/` 配下の日本語Markdownにまとめる
- `private_docs/` 配下の自分用ドキュメントはGit管理しない
